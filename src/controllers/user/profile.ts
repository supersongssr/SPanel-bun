...
export const userProfileController = new Elysia({ prefix: '/user' })
  ...
  .get('/profile', async ({ userId, set }) => {
    ...
    // 查询最近 5 分钟活跃 of IP (aliveIpTable)
  ...
  /**
   * 获取当前登录会员核心资料 (E2E测试/兼容接口)
   */
  .get('/info', async ({ userId, set }) => {
    requireAuth({ userId, set });
  ...
  /**
   * 获取用户流量统计信息 (E2E测试/仪表盘接口)
   */
  .get('/traffic', async ({ userId, set }) => {
    requireAuth({ userId, set });
    ...
    const transferEnable = user.transferEnable;
    const remaining = transferEnable - totalUsed < 0n ? 0n : transferEnable - totalUsed;
    
    const usedPercent = transferEnable > 0n 
      ? Number(totalUsed * 10000n / transferEnable) / 100
      : 0;
